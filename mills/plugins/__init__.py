...
import json
from mills.utils.helpers import insert_gates
from mills.utils.logger import log
from mills.func.tools import create_telegraph
...
rand_user_base = RandUser()

Telegraph_client = create_telegraph()
...
with open('datasets/quotes.json', 'r') as rea:
    QUOTES = json.loads(rea.read())
    if not QUOTES:
        log.error("Quotes Not Imported.")                   
    

log.info("Loaded {} quotes".format(len(QUOTES)))
